...
wantRR = true   --Mi-8--added for JTAC mod   http://forums.eagle.ru/showpost.php?p=2599350&postcount=324
...
ClientAndTankerHandler = {
	make = function(self, message, direction, useIndex)
		if direction  then
			return self.sub.TankerCallsign:make(message.receiver) + comma_space_ + self.sub.PlayerAircraftCallsign:make(message.sender, useIndex) + comma_space_ + Event:make(message.event)
		else
			return self.sub.PlayerAircraftCallsign:make(message.receiver) + comma_space_ + self.sub.TankerCallsign:make(message.sender) + comma_space_ + Event:make(message.event)
		end
	end,
...
AWACSVectorToBullseyeHandler = {
	...
	sub = {	AWACSToClientHandler	= AWACSToClientHandler,
			bulls					= PhraseRandom:new({{_('bulls'),	'at bulls'},
														{_('bullseye'),	'at bullseye'}}),
			Direction				= Direction }
}
...
CAScheckInHandler = {
	make = function(self, message)	
		...
		if message.parameters then
			...
			result = result + self.sub.Digits:make(message.parameters.flight_size) + ' x ' + self.sub.aircraftType:make(message.parameters.aircraft_type[4],custom) + CR_
